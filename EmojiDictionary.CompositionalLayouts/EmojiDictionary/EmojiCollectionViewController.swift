...
//    private var layout: UICollectionViewLayout?
    ...
    private var emojiIdentifiersSnapshot: NSDiffableDataSourceSnapshot<String, Emoji.ID> {
        ...
        let grouped = Dictionary(grouping: emojis, by: { $0.sectionTitle }).sorted(by: { $0.0 < $1.0})
        
        for (title, emojis) in grouped {
            snapshot.appendSections([title])
            let emojiIDs = emojis.sorted(by: { $0.name < $1.name }).map(\.id)
            snapshot.appendItems(emojiIDs)
        }
    ...
    func configureDataSource() {
        ...
        let headerRegsitration = UICollectionView.SupplementaryRegistration<EmojiCollectionViewHeader>(elementKind: headerKind) {
            [weak self] supplementaryView, elementKind, indexPath in
            guard let self else { return }
            
            if let sectionID = collectionDataSource.sectionIdentifier(for: indexPath.section) {
                supplementaryView.titleLabel.text = sectionID
            }
        }
        
        collectionDataSource.supplementaryViewProvider = { collectionView, elementKind, indexPath in collectionView.dequeueConfiguredReusableSupplementary(using: headerRegsitration, for: indexPath)
        }
